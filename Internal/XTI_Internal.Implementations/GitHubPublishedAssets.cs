...
namespace XTI_Internal.Implementations;
...
public sealed class GitHubPublishedAssets : IPublishedAssets
{
    ...
    public GitHubPublishedAssets(XtiGitHubRepository gitHubRepo, string tempDirName)
    {
    ...
    private async Task<string> DownloadSetup(string appTempDir, string appKeyText, GitHubRelease release)
    {
        ...
        var setupAsset = release.Assets
            .FirstOrDefault(a => a.Name.Equals($"{appKeyText}Setup.zip", StringComparison.OrdinalIgnoreCase));
        if (setupAsset != null)
        {
    ...
    private async Task<string> DownloadApp(string appTempDir, string appKeyText, GitHubRelease release)
    {
        ...
        var appAsset = release.Assets
            .FirstOrDefault(a => a.Name.Equals($"{appKeyText}.zip", StringComparison.OrdinalIgnoreCase));
        if (appAsset != null)
        {
