...
{
    public ResourceGroup(IHttpClientFactory httpClientFactory, XtiTokenAccessor xtiTokenAccessor, AppClientUrl clientUrl) : base(httpClientFactory, xtiTokenAccessor, clientUrl, "Resource")
    {
    }
    ...
    public Task<AppLogEntryModel[]> GetMostRecentErrorEvents(string modifier, GetResourceLogRequest model) => Post<AppLogEntryModel[], GetResourceLogRequest>("GetMostRecentErrorEvents", modifier, model);
}