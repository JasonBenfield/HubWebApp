...
public sealed partial class PublishGroup : AppClientGroup
{
    ...
    public Task<XtiVersionModel[]> GetVersions(AppKeyRequest model, CancellationToken ct = default) => Actions.GetVersions.Post("", model, ct);
    public sealed record PublishGroupActions(AppClientPostAction<NewVersionRequest, XtiVersionModel> NewVersion, AppClientPostAction<PublishVersionRequest, XtiVersionModel> BeginPublish, AppClientPostAction<PublishVersionRequest, XtiVersionModel> EndPublish, AppClientPostAction<AppKeyRequest, XtiVersionModel[]> GetVersions);
}