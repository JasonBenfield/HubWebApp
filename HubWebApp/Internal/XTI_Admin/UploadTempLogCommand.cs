...
internal sealed class UploadTempLogCommand : ICommand
{
    ...
    {
        tempLogs = scopes.GetRequiredService<ITempLogs>();
        permanentLogClient = scopes.GetRequiredService<IPermanentLogClient>();
        clock = scopes.GetRequiredService<IClock>();
    }

    public async Task Execute()
    {
        var modifiedBefore = clock.Now().AddMinutes(0);
