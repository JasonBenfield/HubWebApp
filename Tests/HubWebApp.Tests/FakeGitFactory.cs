...
public sealed class FakeGitFactory : GitFactory
{
    ...
    {
        XtiGitHubRepository repo = gitHubRepo ??= new FakeXtiGitHubRepository(ownerName);
        return Task.FromResult(repo);
    }

    private FakeXtiGitRepository? gitRepo;

    public Task<IXtiGitRepository> CreateGitRepo()
    {
        IXtiGitRepository repo = gitRepo ??= new FakeXtiGitRepository();
        return Task.FromResult(repo);
    }
}